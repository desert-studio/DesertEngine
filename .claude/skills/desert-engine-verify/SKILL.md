...
to `.claude/` produced `FAIL BuildVersion` on an otherwise clean 232-suite run, and the suite was
6/6 green the moment `Common` was rebuilt. The sweep runs the WORKING TREE but `BuildVersion` asks
about HEAD, so moving HEAD underneath it makes the instrument answer a question nobody asked.
