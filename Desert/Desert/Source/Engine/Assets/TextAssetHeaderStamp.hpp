...
namespace Desert::Assets
{
    ...
    // A .demat: the material schema, numbered for the first time by AF6f.
    inline constexpr uint32_t kMaterialSchemaTag     = Common::Content::FourCC( "MATL" );
    inline constexpr uint32_t kMaterialSchemaVersion = 1;
    ...
    // so a rename or a move keeps it. Kind and versions are this build's; Dependencies are empty because no
    // text asset yet refers to another by its GUID (references are u64 handles or paths - AF7).
