...
namespace Desert::Core
{
    namespace
    {
        ...
        // A local-space hit distance is not a world one: ToLocalSpace renormalises the direction, so under a
        // scaled transform local t = world t / scale. Hits from different entities — and the landscape,
        // which answers in world centimetres — are compared in world units or the nearest is not nearest.
        float WorldDistance( const Common::Math::Ray& ray, const glm::mat4& xf, const Common::Math::Ray& localRay,
                             float localT )
        {
            const glm::vec3 world = glm::vec3( xf * glm::vec4( localRay.GetPoint( localT ), 1.0f ) );
            return glm::length( world - ray.Origin );
        }
        ...
        for ( const auto& entity : GetAllEntities() )
        {
            ...
            if ( !skinned )
            {
                for ( const auto& sm : mesh->GetSubmeshes() )
                {
                    ...
                    if ( !localRay.IntersectsAABB( sm.BoundingBox, t ) || t <= 0.0f )
                        continue;
                    const float worldT = WorldDistance( ray, xf, localRay, t );
                    if ( worldT < closest )
                    {
                        closest    = worldT;
                        bestXf     = xf;
            ...
            else
            {
                ...
                if ( localRay.IntersectsAABB( bounds, t ) && t > 0.0f &&
                     WorldDistance( ray, xf, localRay, t ) < closest )
                {
                    closest    = WorldDistance( ray, xf, localRay, t );
                    bestXf     = xf;
                    bestAABB   = bounds;
                    bestLocal  = localRay;
                    bestLocalT = t;
