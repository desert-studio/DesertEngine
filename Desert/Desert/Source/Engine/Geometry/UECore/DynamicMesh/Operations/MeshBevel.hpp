...
// ApplyProfileShape_Round B:3241-3739) is ported for loops, edges, 4-sided and 3-sided (PN-triangle) junction
// patches; the valence-3 barycentrics come from the tessellation lattice instead of UE's vertex colours. UE's
// valence >= 5 MVC patch (and DeformNormals, which only it reads) is not, so a round profile with such a junction
// is refused before any edit. FInterpCurveVector becomes the two-point cubic
...
namespace Desert::Geometry
{
    ...
    public:
         ...
         * subdivisions refuses a junction of 5+ bevelled edges (its MVC patch is not ported).
         */
        ...
        struct FBevelVertex_InteriorVertex
        {
            ...
            // (w, u, v), its barycentrics of InteriorBorderLoop[0..2]
        ...
        struct FBevelVertex
        {
            ...
            // multi-segment 3- or 4-sided junction polygon: its added interior vertices, and its corners in the
            // order c00, c10, c01, c11 (quad) or in wedge order (triangle); read by the round profile
    ...
    protected:
         ...
         *  then each 4-sided junction patch by blending its four border arcs and each 3-sided one as the PN
         *  triangle of its three. */
        void ApplyProfileShape_Round( FDynamicMesh3& Mesh );
        /** A round profile with subdivisions refuses a junction of 5+ bevelled edges before any edit. */
