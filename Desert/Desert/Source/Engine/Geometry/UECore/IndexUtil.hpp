// Ported from UE 5.8 Engine/Source/Runtime/GeometryCore/Public/Util/IndexUtil.h:17-241, adapted: namespace
// IndexUtil nested in Desert::Geometry; the ApplyMap overloads (242-339) need IntVectorTypes and are not ported.
#pragma once
    ...
    namespace IndexUtil
    {
        ...
        inline int FindSharedEdgeVertex( const Index2i& EdgeVerts1, const Index2i& EdgeVerts2 )
        {
            if ( EdgeVerts1.A == EdgeVerts2.A )
                return EdgeVerts1.A;
            else if ( EdgeVerts1.A == EdgeVerts2.B )
                return EdgeVerts1.A;
            else if ( EdgeVerts1.B == EdgeVerts2.A )
                return EdgeVerts1.B;
            else if ( EdgeVerts1.B == EdgeVerts2.B )
                return EdgeVerts1.B;
            else
        ...
        {
            if ( TriangleVerts[0] == VertexID )
                return 0;
            if ( TriangleVerts[1] == VertexID )
                return 1;
            if ( TriangleVerts[2] == VertexID )
                return 2;
            return IndexConstants::InvalidID;
        }
        ...
        {
            if ( SamePairUnordered( VertexID1, VertexID2, TriangleVerts[0], TriangleVerts[1] ) )
                return 0;
            if ( SamePairUnordered( VertexID1, VertexID2, TriangleVerts[1], TriangleVerts[2] ) )
                return 1;
            if ( SamePairUnordered( VertexID1, VertexID2, TriangleVerts[2], TriangleVerts[0] ) )
                return 2;
            return IndexConstants::InvalidID;
        }
        ...
        {
            if ( TriangleVerts[0] == VertexID1 && TriangleVerts[1] == VertexID2 )
                return 0;
            if ( TriangleVerts[1] == VertexID1 && TriangleVerts[2] == VertexID2 )
                return 1;
            if ( TriangleVerts[2] == VertexID1 && TriangleVerts[0] == VertexID2 )
                return 2;
            return IndexConstants::InvalidID;
        }

        /**
         * Find third vertex of triangle that is not VertexID1 or VertexID2. Triangle must contain VertexID1 and
         * VertexID2 or result will be incorrect.
        ...
        /**
         * Find ordered edge [VertexID1,VertexID2] in a triangle that is in an array of triangles, and return
         * remaining third vertex
        ...
        /**
         * Find ordered edge [VertexID1,VertexID2] in TriangleVerts and then return the index of the remaining
         * third vertex
        ...
        /**
         * Assuming [Vertex1,Vertex2] is an unordered edge in TriangleVerts, return Vertex1 and Vertex2 in the
         * correct order (ie the same as TriangleVerts)
        ...
        /**
         * Assuming [Vertex1,Vertex2] is an unordered edge in TriangleVerts, return Vertex1 and Vertex2 in the
         * correct order (ie the same as TriangleVerts), and returns the vertex ID of the other vertex
