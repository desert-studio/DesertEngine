...
// Engine/Source/Runtime/GeometryCore/Public/EdgeLoop.h and EdgeSpan.h (the Vertices/Edges payload and
// InitializeFromVertices/InitializeFromEdges only), adapted: UE Core via UECore.hpp, namespace Desert::Geometry,
// FIndexFlagSet is a TArray<bool> over the ID range. Bowtie vertices (a region vertex touching more than two
// region boundary edges) are refused with a named error instead of being split by UE's
// FindLeftTurnEdge/TryExtractSubloops: no Modeling mesh the editor produces has one, and a refused region is
// reported, never mis-walked.
#pragma once
