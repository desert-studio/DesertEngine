...
// MatrixSolver.h (FLDLTMatrixSolver), adapted: UE's Eigen types are replaced by a minimal CSR matrix and an envelope
// LDLT with reverse Cuthill-McKee ordering (the tree has no Eigen); only the spectral solver is ported (the
// least-squares one has no caller here); constraints are just the free-boundary vertex set, because the spectral
// solve reads only which vertices are constrained, never their positions or weights; the power method starts from
// a fixed-seed vector instead of a fresh random seed, so a solve is reproducible.
#pragma once
...
namespace Desert::Geometry
{
    ...
    public:
        ...
        int32 Cols() const { return NumCols; }

        void Multiply( const std::vector<double>& In, std::vector<double>& Out ) const;
        /** Out = Transpose(this) * In. */
        void MultiplyTransposed( const std::vector<double>& In, std::vector<double>& Out ) const;
    ...
    public:
        ...
        bool Factorize( const FSparseMatrixD& Matrix );
     ...
     * (E_c, B - E E^T) with the smallest eigenvalue, where E_c is the conformal energy (Dirichlet minus signed area),
     * B selects the free-boundary vertices and E their centroid; nothing pins the boundary.
     */
