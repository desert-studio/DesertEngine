...
namespace
{
    ...
    constexpr std::array kPathOnly = {
         ...
         PathOnlyRow{ ContentKind::Shader,
                      "no header format, and MaterialShaderRef (MaterialData.hpp) writes the path alone (AF10b "
                      "redirector row)" },
    ...
    std::vector<char> SyntheticMesh( bool skinned, const AssetGuid& guid )
    {
        ...
        char*          rowAt       = bytes.data() + Common::Content::kMeshBinaryPrefixV3;
        std::memcpy( rowAt, &id, 4 );
