...
TEST( OffsetMeshRegion, CubeFaceExtrudeMatchesUETopology )
{
    ...
    for ( int32_t t : Op.OffsetRegions[0].OffsetTids )
    {
    ...
    for ( int32_t t : Op.OffsetRegions[0].StitchTriangles[0] )
    {
...
TEST( InsetMeshRegion, CubeFaceInsetShrinksByTheDistance )
{
    ...
    EXPECT_EQ( CountBoundaryEdges( M ), 0 );
    for ( int32_t t : Op.InsetRegions[0].InitialTriangles )
    {
    ...
    for ( int32_t t : Op.InsetRegions[0].StitchTriangles[0] )
