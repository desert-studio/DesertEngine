...
namespace Desert::Tests::PointerCensus
{
    ...
    inline const std::vector<Row>& Register()
    {
        ...
        { "Desert/Desert/Source/Engine/Core/Serialize/WorldCells.hpp",
          ...
          "the index the source reads cells against; its owner holds both -- AssembleWorld keeps the source a function-local beside the index it was handed, and WP9's streamer will own the index next to the source. The constructor takes a reference, so a null index cannot be expressed" },
