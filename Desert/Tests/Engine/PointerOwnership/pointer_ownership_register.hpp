...
namespace Desert::Tests::PointerCensus
{
    ...
    inline const std::vector<Row>& Register()
    {
        ...
        { "Desert/Desert/Source/Engine/Geometry/DynamicMeshSelection.cpp",
          "FDynamicMeshElements", "m_Topology", Guard::CallScoped,
          "an argument pack built at the call site (FDynamicMeshElements(mesh, &topology)) where topology is a local FGroupTopology the same caller built and holds; consumed inside that call and dead at the semicolon, same shape as UECore's BaseIterator::Vector rows" },
        { "Desert/Desert/Source/Engine/Geometry/UECore/DynamicMesh/DynamicAttribute.hpp",
          "TDynamicAttributeSetBase", "RegisteredAttributes", Guard::OwnedByThisObject,
          "ported UE code (P8a, GeometryCore), kept in UE's shape: the attributes a set registers are its own members or held by its own unique_ptr layers, unregistered before they are freed; the list is a non-owning index over them" },
        { "Desert/Desert/Source/Engine/Geometry/UECore/DynamicMesh/DynamicMeshAttributeSet.hpp",
          "FDynamicMeshAttributeSet", "ParentMesh", Guard::ObservedContainsUs,
          "ported UE code (P8a, GeometryCore), kept in UE's shape: non-owning back-reference to the FDynamicMesh3 whose unique_ptr AttributeSet this is; the mesh cannot go first" },
        { "Desert/Desert/Source/Engine/Geometry/UECore/DynamicMesh/DynamicMeshOverlay.hpp",
          "TDynamicMeshOverlay", "ParentMesh", Guard::ObservedContainsUs,
          "ported UE code (P8a, GeometryCore), kept in UE's shape: non-owning back-reference to the mesh whose attribute set holds this overlay; re-pointed by Reparent when the set is copied" },
        { "Desert/Desert/Source/Engine/Geometry/UECore/DynamicMesh/DynamicMeshTriangleAttribute.hpp",
          "TDynamicMeshTriangleAttribute", "ParentMesh", Guard::ObservedContainsUs,
          "ported UE code (P8a, GeometryCore), kept in UE's shape: non-owning back-reference to the mesh whose attribute set holds this layer; re-pointed by Reparent when the set is copied" },
        { "Desert/Desert/Source/Engine/Geometry/UECore/DynamicMesh/MeshNormals.hpp",
          "FMeshNormals", "Mesh", Guard::HostOutlivesUs,
          "ported UE code (P8a, GeometryCore), kept in UE's shape: a normals computation built over a mesh the caller holds for the computation's lifetime; never stored beyond it" },
        { "Desert/Desert/Source/Engine/Geometry/UECore/Polygroups/PolygroupSet.hpp",
          "FPolygroupSet", "Mesh", Guard::HostOutlivesUs,
          "ported UE code (P8a, GeometryCore), kept in UE's shape: a view of one mesh's polygroups, built over a mesh the caller holds for the view's lifetime" },
        { "Desert/Desert/Source/Engine/Geometry/UECore/Polygroups/PolygroupSet.hpp",
          "FPolygroupSet", "PolygroupAttrib", Guard::HostOutlivesUs,
          "ported UE code (P8a, GeometryCore), kept in UE's shape: a layer inside the attribute set of the same mesh as FPolygroupSet::Mesh, held for the view's lifetime by the same caller" },
        { "Desert/Desert/Source/Engine/Geometry/UECore/RefCountVector.hpp",
          "BaseIterator", "Vector", Guard::CallScoped,
          "ported UE code (P8a, GeometryCore), kept in UE's shape: an iterator over the FRefCountVector it came from, consumed by the loop that made it (non-owning)" },
        { "Desert/Desert/Source/Engine/Geometry/UECore/RefCountVector.hpp",
          "IndexEnumerable", "Vector", Guard::CallScoped,
          "ported UE code (P8a, GeometryCore), kept in UE's shape: a range-for enumerable over the FRefCountVector it came from, consumed by that loop (non-owning)" },
        { "Desert/Desert/Source/Engine/Geometry/UECore/SmallListSet.hpp",
          "BaseValueIterator", "ListSet", Guard::CallScoped,
          "ported UE code (P8a, GeometryCore), kept in UE's shape: an iterator over the FSmallListSet it came from, consumed by the loop that made it (non-owning)" },
        { "Desert/Desert/Source/Engine/Geometry/UECore/SmallListSet.hpp",
          "ValueEnumerable", "ListSet", Guard::CallScoped,
          "ported UE code (P8a, GeometryCore), kept in UE's shape: a range-for enumerable over the FSmallListSet it came from, consumed by that loop (non-owning)" },
        { "Desert/Desert/Source/Engine/Geometry/UECore/SmallListSet.hpp",
          "MappedValueEnumerable", "ListSet", Guard::CallScoped,
          "ported UE code (P8a, GeometryCore), kept in UE's shape: a range-for enumerable over the FSmallListSet it came from, consumed by that loop (non-owning)" },
        { "Desert/Desert/Source/Engine/Geometry/UECore/DynamicMesh/GroupTopology.hpp",
          "FGroupTopology", "Mesh", Guard::HostOutlivesUs,
          "ported UE code (P12, GeometryCore), kept in UE's shape: the constructor takes the mesh it is built over (const FDynamicMesh3*, non-owning) and every caller builds a topology on a mesh it already holds, for the topology's lifetime" },
        { "Desert/Desert/Source/Engine/Geometry/UECore/DynamicMesh/MeshTangents.hpp",
          "TMeshTangents", "Mesh", Guard::HostOutlivesUs,
          "ported UE code (P12, GeometryCore), kept in UE's shape: a tangents computation built over a mesh the caller holds for the computation's lifetime; never stored beyond it" },
        { "Desert/Desert/Source/Engine/Geometry/UECore/DynamicMesh/Operations/InsetMeshRegion.hpp",
          "FInsetMeshRegion", "Mesh", Guard::HostOutlivesUs,
          "ported UE code (P12, GeometryCore), kept in UE's shape: an inset operation built at the call site over the caller's FDynamicMesh3* and applied in-place; the operation object does not outlive that call" },
        { "Desert/Desert/Source/Engine/Geometry/UECore/DynamicMesh/Operations/MergeCoincidentMeshEdges.hpp",
          "FMergeCoincidentMeshEdges", "Mesh", Guard::HostOutlivesUs,
          "ported UE code (P12, GeometryCore), kept in UE's shape: the constructor takes the mesh it merges edges of (FDynamicMesh3*, non-owning); built at the call site and applied in-place, never stored beyond that call" },
        { "Desert/Desert/Source/Engine/Geometry/UECore/DynamicMesh/Operations/MergeCoincidentMeshEdges.hpp",
          "FMergeCoincidentMeshEdges", "EdgesToMerge", Guard::HostOutlivesUs,
          "ported UE code (P12, GeometryCore), kept in UE's shape: optional, caller-owned TSet<int32>* naming the edge subset to merge (null means the whole mesh); the caller's set outlives the merge call that reads it" },
        { "Desert/Desert/Source/Engine/Geometry/UECore/DynamicMesh/Operations/OffsetMeshRegion.hpp",
          "FOffsetMeshRegion", "Mesh", Guard::HostOutlivesUs,
          "ported UE code (P12, GeometryCore), kept in UE's shape: an offset/extrude operation built at the call site over the caller's FDynamicMesh3* and applied in-place; the operation object does not outlive that call" },
        { "Desert/Desert/Source/Engine/Geometry/UECore/DynamicMesh/Operations/SimpleHoleFiller.hpp",
          "FSimpleHoleFiller", "Mesh", Guard::HostOutlivesUs,
          "ported UE code (P12, GeometryCore), kept in UE's shape: the hole filler is built at the call site over the caller's FDynamicMesh3* and fills in-place; the filler object does not outlive that call" },
        { "Desert/Desert/Source/Engine/Geometry/UECore/Operations/EmbedSurfacePath.hpp",
          "FMeshSurfacePath", "Mesh", Guard::HostOutlivesUs,
          "ported UE code (P12g, GeometryCore), kept in UE's shape: the constructor takes the mesh the path lies on (FDynamicMesh3*, non-owning); built at the call site, embedded into that mesh in-place, and never stored beyond the call" },
        { "Desert/Desert/Source/Engine/Geometry/UECore/DynamicMesh/Operations/GroupEdgeInserter.hpp",
          "FEdgeLoopInsertionParams", "Mesh", Guard::CallScoped,
          "ported UE code (P12h, GeometryCore), kept in UE's shape: an FGroupEdgeInserter argument pack built at the call site; the caller's FDynamicMesh3* (non-owning) that InsertEdgeLoops edits in-place" },
        { "Desert/Desert/Source/Engine/Geometry/UECore/DynamicMesh/Operations/GroupEdgeInserter.hpp",
          "FEdgeLoopInsertionParams", "Topology", Guard::CallScoped,
          "ported UE code (P12h, GeometryCore), kept in UE's shape: an FGroupEdgeInserter argument pack built at the call site; the caller's FGroupTopology* (non-owning) over that mesh, rebuilt in-place by InsertEdgeLoops" },
        { "Desert/Desert/Source/Engine/Geometry/UECore/DynamicMesh/Operations/GroupEdgeInserter.hpp",
          "FEdgeLoopInsertionParams", "SortedInputLengths", Guard::CallScoped,
          "ported UE code (P12h, GeometryCore), kept in UE's shape: an FGroupEdgeInserter argument pack built at the call site; the caller's sorted TArray<double> of loop positions (non-owning), read during InsertEdgeLoops only" },
        { "Desert/Desert/Source/Engine/Geometry/UECore/DynamicMesh/Operations/GroupEdgeInserter.hpp",
          "FGroupEdgeInserterOptionalOutputParams", "NewEidsOut", Guard::CallScoped,
          "ported UE code (P12h, GeometryCore), kept in UE's shape: an FGroupEdgeInserter argument pack built at the call site; optional caller-owned TSet<int32>* out-parameter (null means not wanted), written during the insert call only" },
        { "Desert/Desert/Source/Engine/Geometry/UECore/DynamicMesh/Operations/GroupEdgeInserter.hpp",
          "FGroupEdgeInserterOptionalOutputParams", "ChangedTidsOut", Guard::CallScoped,
          "ported UE code (P12h, GeometryCore), kept in UE's shape: an FGroupEdgeInserter argument pack built at the call site; optional caller-owned TSet<int32>* out-parameter (null means not wanted), written during the insert call only" },
        { "Desert/Desert/Source/Engine/Geometry/UECore/DynamicMesh/Operations/GroupEdgeInserter.hpp",
          "FGroupEdgeInserterOptionalOutputParams", "ProblemGroupEdgeIDsOut", Guard::CallScoped,
          "ported UE code (P12h, GeometryCore), kept in UE's shape: an FGroupEdgeInserter argument pack built at the call site; optional caller-owned TSet<int32>* out-parameter (null means not wanted), written during InsertEdgeLoops only" },
        { "Desert/Desert/Source/Engine/Geometry/UECore/DynamicMesh/Operations/GroupEdgeInserter.hpp",
          "FGroupEdgeInsertionParams", "Mesh", Guard::CallScoped,
          "ported UE code (P12h, GeometryCore), kept in UE's shape: an FGroupEdgeInserter argument pack built at the call site; the caller's FDynamicMesh3* (non-owning) that InsertGroupEdge edits in-place" },
        { "Desert/Desert/Source/Engine/Geometry/UECore/DynamicMesh/Operations/GroupEdgeInserter.hpp",
          "FGroupEdgeInsertionParams", "Topology", Guard::CallScoped,
          "ported UE code (P12h, GeometryCore), kept in UE's shape: an FGroupEdgeInserter argument pack built at the call site; the caller's FGroupTopology* (non-owning) over that mesh, rebuilt in-place by InsertGroupEdge" },
        { "Desert/Desert/Source/Engine/Geometry/UECore/DynamicMeshEditor.hpp",
          "FDynamicMeshEditor", "Mesh", Guard::HostOutlivesUs,
          "ported UE code (P12, GeometryCore), kept in UE's shape: the constructor takes the mesh it edits (FDynamicMesh3*, non-owning); built at the call site for one edit and applied in-place, never stored beyond it" },
        { "Desert/Desert/Source/Engine/Geometry/UECore/MeshBoundaryLoops.hpp",
          "FMeshBoundaryLoops", "Mesh", Guard::HostOutlivesUs,
          "ported UE code (P12, GeometryCore), kept in UE's shape: a boundary-loop walk built over a mesh the caller holds for the walk's lifetime; never stored beyond it" },
        { "Desert/Desert/Source/Engine/Geometry/UECore/MeshRegionBoundaryLoops.hpp",
          "FMeshRegionBoundaryLoops", "Mesh", Guard::HostOutlivesUs,
          "ported UE code (P12, GeometryCore), kept in UE's shape: a region boundary-loop walk built over a mesh the caller holds for the walk's lifetime; never stored beyond it" },
        ...
        { "Desert/Desert/Source/Engine/Graphic/Systems/Scene/Terrain/TerrainBatch.hpp",
          "LandscapeWeightDraw", "Weightmap", Guard::FrameScoped,
          "the tile's RGBA8 weight copy, held by LandscapeECSSystem::TileGpu::Weightmap for longer than the frame and rewritten in place per stroke; dropping it releases the shared_ptr into the allocator's per-frame deletion queue, never frees it under a recorded frame" },
        ...
        { "Desert/Desert/Source/Engine/World/Landscape/LandscapeData.hpp",
          "LandscapeTileNeighbours", "West", Guard::CallScoped,
          "the tiles beside the one sampled, borrowed for one LandscapeBorderedSamples / SampleLandscapeNormal call from the caller's own tile components (LandscapeECSSystem, the tests); null = not loaded" },
        { "Desert/Desert/Source/Engine/World/Landscape/LandscapeData.hpp",
          "LandscapeTileNeighbours", "East", Guard::CallScoped,
          kWhyArgumentPack },
        { "Desert/Desert/Source/Engine/World/Landscape/LandscapeData.hpp",
          "LandscapeTileNeighbours", "South", Guard::CallScoped,
          kWhyArgumentPack },
        { "Desert/Desert/Source/Engine/World/Landscape/LandscapeData.hpp",
          "LandscapeTileNeighbours", "North", Guard::CallScoped,
          kWhyArgumentPack },
        ...
        { "Editor/Source/Editor/Panels/Collections/CollectionsPanel.hpp",
          ...
          "EditorLayer owns it and declares it BEFORE m_Panels, so the panels are destroyed first (members die in reverse declaration order); EditorLayerDeclaresItsHostsBeforeItsPanels asserts that order rather than trusting it" },
        { "Editor/Source/Editor/Panels/WorldPartition/WorldPartitionPanel.hpp",
          "WorldPartitionPanel", "m_Assets", Guard::HostOutlivesUs,
          "EditorLayer owns the AssetManager and declares it BEFORE m_Panels, so the panel is destroyed first (members die in reverse declaration order); EditorLayerDeclaresItsHostsBeforeItsPanels asserts that order rather than trusting it" },
        ...
        { "Editor/Source/Editor/Panels/FileExplorer/FileExplorerPanel.hpp",
          ...
          "EditorLayer owns it and declares it BEFORE m_Panels, so the panels are destroyed first (members die in reverse declaration order); EditorLayerDeclaresItsHostsBeforeItsPanels asserts that order rather than trusting it" },
        { "Editor/Source/Editor/Panels/FileExplorer/FileExplorerPanel.hpp",
          "FileExplorerPanel", "m_TooltipEntry", Guard::OwnedByThisObject,
          "UI1 (asset info popup): DrawAssetTooltip(entry) points at a node in this panel's own m_Directories, same as m_CurrentSelected; cleared to nullptr the frame the tile stops being hovered (DrawAssetTile: `else if (m_TooltipEntry == &*entry) m_TooltipEntry = nullptr;`), so it never outlives the frame in which its node was live" },
        ...
        { "Editor/Source/Editor/Panels/Modeling/ModelingPanel.cpp",
          "PaletteEntry", "Icon", Guard::StaticStorage,
          "P12 renamed the palette rail's entry struct from Cat to PaletteEntry (kPalettes); still a string literal held by a constexpr/static table entry" },
        { "Editor/Source/Editor/Panels/Modeling/ModelingPanel.cpp",
          "PaletteEntry", "Name", Guard::StaticStorage,
          "P12 renamed the palette rail's entry struct from Cat to PaletteEntry (kPalettes); still a string literal held by a constexpr/static table entry" },
        ...
        { "Desert/Desert/Source/Engine/Graphic/ViewResources.hpp",
          "ActiveViewScope", "m_Previous", Guard::HostOutlivesUs,
          "the view that was active when the scope opened (null = the frame context). Owned by its SceneRenderer; the scope never dereferences it, and RestoreActive re-arms it only if it is still on the live list, so a view destroyed inside the scope restores the frame context instead of a dangling pointer (pinned by AViewDestroyedWhileActiveHandsBackToTheFrameContext)" },
        ...
        { "Desert/Desert/Source/Engine/Core/WorldStreamer.hpp",
          "WorldStreamer", "m_Scene", Guard::HostOutlivesUs,
          "the scene being played; the streamer is EditorLayer::m_WorldStreamer and exists only during Play. The ORDER is named at both places a playing scene ends: OnSceneStop resets the streamer before Scene::Clear, and closing the playing document resets it with the snapshot before the scene is torn down. The destructor reads no scene state -- it only logs its own counters" },
        { "Desert/Desert/Source/Engine/Core/WorldStreamer.hpp",
          "WorldStreamer", "m_Assets", Guard::HostOutlivesUs,
          "EditorLayer::m_AssetManager, which lives for the whole editor session; a Play session is strictly inside it" },
        { "Desert/Desert/Source/Engine/Core/Serialize/WorldCells.hpp",
          "CookedCellSource", "m_Index", Guard::HostOutlivesUs,
          "the index the source reads cells against; its owner holds both -- AssembleWorld keeps the source a function-local beside the index it was handed, and WorldStreamer holds the index (m_Index, shared) declared before the loader that owns the source, whose destructor waits for every read in flight. The constructor takes a reference, so a null index cannot be expressed" },
        ...
        { "Desert/Desert/Source/Engine/ECS/System/LandscapeCollision.hpp",
          "LandscapeCollision", "m_World", Guard::HostOutlivesUs,
          "PhysicsECSSystem declares m_Landscape AFTER m_World, so it is destroyed first (its destructor removes its bodies through this), and resets it explicitly before m_World->Shutdown()" },
        { "Desert/Desert/Source/Engine/ECS/System/LandscapeCollision.hpp",
          "LandscapeCollision", "m_Registry", Guard::ObservedContainsUs,
          "the registry is a member of the Scene that owns the PhysicsECSSystem that owns this; Detach() in the destructor disconnects through it while it is alive" },
        { "Desert/Desert/Source/Engine/ECS/LandscapeRootOf.hpp",
          "LandscapeTileRef", "Component", Guard::CallScoped,
          "DrawableLandscapeTiles fills it from the registry and PhysicsECSSystem hands the vector straight to LandscapeCollision::Sync in the same expression; Sync adds and removes no component, so the entt pool element it points at cannot move or die inside the call" },
        { "Desert/Desert/Source/Engine/World/Landscape/LandscapeRaycast.hpp",
          "LandscapeRayTile", "Heights", Guard::CallScoped,
          "the argument pack of RaycastLandscape: Scene::Raycast builds it from the tiles' components and consumes it in the same call; nothing in the ray query touches the registry" },
        { "Desert/Desert/Source/Engine/World/Landscape/LandscapeEditCache.hpp",
          "LandscapeTileSlot", "Data", Guard::CallScoped,
          "the answer of a LandscapeTileLookup: the lookup reads it out of the tile's registry component and the cache uses it inside the same Read/Write call; a cache never keeps a slot across calls and never adds or removes a component, so the entt pool element cannot move or die while it is held" },
        { "Editor/Source/Editor/Core/Selection/LandscapeSculptState.hpp",
          "LandscapeToolControl", "Row", Guard::StaticStorage,
          "a string literal in the per-tool control table" },
        { "Editor/Source/Editor/Core/Selection/LandscapeSculptState.hpp",
          "LandscapeToolProperty", "Label", Guard::StaticStorage,
          "a string literal in the per-tool property table" },
        { "Editor/Source/Editor/Panels/Landscape/LandscapePanel.cpp",
          "ToolButton", "Icon", Guard::StaticStorage,
          "an ICON_MDI_* string literal in the panel's static tool-strip table" },
        { "Editor/Source/Editor/Panels/ViewportPanel/Tools/ActiveToolBar.hpp",
          "ActiveToolLabel", "Icon", Guard::StaticStorage,
          "P12: LabelOf() returns an ICON_MDI_* string literal per active tool; static storage duration" },
        { "Editor/Source/Editor/Panels/ViewportPanel/Tools/ActiveToolBar.hpp",
          "ActiveToolLabel", "Name", Guard::StaticStorage,
          "P12: LabelOf() returns a string literal per active tool (\"CubeGrid\", \"PolyEdit\", ...); static storage duration" },
        { "Editor/Source/Editor/Panels/ViewportPanel/Tools/LandscapeSculptTool.cpp",
          "LandscapeStrokeCommand", "m_Scene", Guard::HostOutlivesUs,
          "CommandHistory::Clear() runs on scene load and on entering Play, so no entry outlives the scene it was recorded in; the landscape itself is re-found by UUID on every Undo/Redo and a missing one refuses with a toast" },
        { "Editor/Source/Editor/Panels/ViewportPanel/Tools/LandscapePaintTool.cpp",
          "LandscapePaintCommand", "m_Scene", Guard::HostOutlivesUs,
          "the paint twin of LandscapeStrokeCommand::m_Scene: CommandHistory::Clear() runs on scene load and on entering Play, so no entry outlives the scene it was recorded in; the landscape is re-found by UUID on every Undo/Redo" },
        { "Editor/Source/Editor/Panels/ViewportPanel/Tools/LandscapeSculptTool.hpp",
          "LandscapeSculptTool", "m_ToolName", Guard::StaticStorage,
          "the label of a LandscapeTool, a string literal from the tool table, fixed when the stroke begins" },
