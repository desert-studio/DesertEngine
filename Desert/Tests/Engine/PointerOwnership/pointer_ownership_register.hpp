...
namespace Desert::Tests::PointerCensus
{
    ...
    inline const std::vector<Row>& Register()
    {
        ...
        { "Desert/Desert/Source/Engine/Geometry/DynamicMeshSelection.cpp",
          "DynamicMeshElements", "m_Topology", Guard::CallScoped,
          "an argument pack built at the call site (DynamicMeshElements(mesh, &topology)) where topology is a local GroupTopology the same caller built and holds; consumed inside that call and dead at the semicolon, same shape as UECore's BaseIterator::Vector rows" },
        { "Desert/Desert/Source/Engine/Geometry/UECore/DynamicMesh/DynamicAttribute.hpp",
          "DynamicAttributeSetBase", "m_RegisteredAttributes", Guard::OwnedByThisObject,
        ...
        { "Desert/Desert/Source/Engine/Geometry/UECore/DynamicMesh/DynamicMeshAttributeSet.hpp",
          "DynamicMeshAttributeSet", "m_ParentMesh", Guard::ObservedContainsUs,
          "ported UE code (P8a, GeometryCore), kept in UE's shape: non-owning back-reference to the DynamicMesh3 whose unique_ptr AttributeSet this is; the mesh cannot go first" },
        { "Desert/Desert/Source/Engine/Geometry/UECore/DynamicMesh/DynamicMeshOverlay.hpp",
          "DynamicMeshOverlay", "m_ParentMesh", Guard::ObservedContainsUs,
        ...
        { "Desert/Desert/Source/Engine/Geometry/UECore/DynamicMesh/DynamicMeshTriangleAttribute.hpp",
          "DynamicMeshTriangleAttribute", "m_ParentMesh", Guard::ObservedContainsUs,
        ...
        { "Desert/Desert/Source/Engine/Geometry/UECore/DynamicMesh/Parameterization/DynamicMeshUVEditor.hpp",
          "DynamicMeshUVEditor", "m_Mesh", Guard::HostOutlivesUs,
          "ported UE code (P13d, DynamicMesh), kept in UE's shape: a UV editor built over a mesh the caller holds for the edit's lifetime (MeshBevel::ComputeUVs); never stored beyond it" },
        { "Desert/Desert/Source/Engine/Geometry/UECore/DynamicMesh/Parameterization/DynamicMeshUVEditor.hpp",
          "DynamicMeshUVEditor", "m_UVOverlay", Guard::HostOutlivesUs,
          "ported UE code (P13d, DynamicMesh): the edited mesh's own UV overlay, owned by that mesh's attribute set; the mesh outlives the editor as above" },
        { "Desert/Desert/Source/Engine/Geometry/UECore/DynamicSubmesh3.hpp",
          "DynamicSubmesh3", "m_BaseMesh", Guard::HostOutlivesUs,
          "ported UE code (P13d, GeometryCore): a submesh extracted from a base mesh the caller holds while the submesh is used to map results back" },
        { "Desert/Desert/Source/Engine/Geometry/UECore/Parameterization/MeshDijkstra.hpp",
          "MeshDijkstra", "m_PointSet", Guard::HostOutlivesUs,
          "ported UE code (P13d, GeometryCore): a graph search over a point set (the submesh) the caller holds for the search's lifetime; stack-local in ComputeArbitraryTrianglePatchUVs" },
        { "Desert/Desert/Source/Engine/Geometry/UECore/Parameterization/MeshLocalParam.hpp",
          "MeshLocalParam", "m_PointSet", Guard::HostOutlivesUs,
          "ported UE code (P13d, GeometryCore): the exponential map's point set, held by the caller for the parameterisation's lifetime; stack-local like MeshDijkstra" },
        { "Desert/Desert/Source/Engine/Geometry/UECore/DynamicMesh/MeshNormals.hpp",
          "MeshNormals", "m_Mesh", Guard::HostOutlivesUs,
        ...
        { "Desert/Desert/Source/Engine/Geometry/UECore/Polygroups/PolygroupSet.hpp",
          "PolygroupSet", "Mesh", Guard::HostOutlivesUs,
        ...
        { "Desert/Desert/Source/Engine/Geometry/UECore/Polygroups/PolygroupSet.hpp",
          "PolygroupSet", "PolygroupAttrib", Guard::HostOutlivesUs,
          "ported UE code (P8a, GeometryCore), kept in UE's shape: a layer inside the attribute set of the same mesh as PolygroupSet::Mesh, held for the view's lifetime by the same caller" },
        { "Desert/Desert/Source/Engine/Geometry/UECore/RefCountVector.hpp",
          "BaseIterator", "m_Vector", Guard::CallScoped,
          "ported UE code (P8a, GeometryCore), kept in UE's shape: an iterator over the RefCountVector it came from, consumed by the loop that made it (non-owning)" },
        { "Desert/Desert/Source/Engine/Geometry/UECore/RefCountVector.hpp",
          "IndexEnumerable", "m_Vector", Guard::CallScoped,
          "ported UE code (P8a, GeometryCore), kept in UE's shape: a range-for enumerable over the RefCountVector it came from, consumed by that loop (non-owning)" },
        { "Desert/Desert/Source/Engine/Geometry/UECore/SmallListSet.hpp",
          "BaseValueIterator", "m_ListSet", Guard::CallScoped,
          "ported UE code (P8a, GeometryCore), kept in UE's shape: an iterator over the SmallListSet it came from, consumed by the loop that made it (non-owning)" },
        { "Desert/Desert/Source/Engine/Geometry/UECore/SmallListSet.hpp",
          "ValueEnumerable", "m_ListSet", Guard::CallScoped,
          "ported UE code (P8a, GeometryCore), kept in UE's shape: a range-for enumerable over the SmallListSet it came from, consumed by that loop (non-owning)" },
        { "Desert/Desert/Source/Engine/Geometry/UECore/SmallListSet.hpp",
          "MappedValueEnumerable", "m_ListSet", Guard::CallScoped,
          "ported UE code (P8a, GeometryCore), kept in UE's shape: a range-for enumerable over the SmallListSet it came from, consumed by that loop (non-owning)" },
        { "Desert/Desert/Source/Engine/Geometry/UECore/DynamicMesh/GroupTopology.hpp",
          "GroupTopology", "m_Mesh", Guard::HostOutlivesUs,
          "ported UE code (P12, GeometryCore), kept in UE's shape: the constructor takes the mesh it is built over (const DynamicMesh3*, non-owning) and every caller builds a topology on a mesh it already holds, for the topology's lifetime" },
        { "Desert/Desert/Source/Engine/Geometry/UECore/DynamicMesh/MeshTangents.hpp",
          "MeshTangents", "m_Mesh", Guard::HostOutlivesUs,
        ...
        { "Desert/Desert/Source/Engine/Geometry/UECore/DynamicMesh/Operations/InsetMeshRegion.hpp",
          "InsetMeshRegion", "m_Mesh", Guard::HostOutlivesUs,
          "ported UE code (P12, GeometryCore), kept in UE's shape: an inset operation built at the call site over the caller's DynamicMesh3* and applied in-place; the operation object does not outlive that call" },
        { "Desert/Desert/Source/Engine/Geometry/UECore/DynamicMesh/Operations/MergeCoincidentMeshEdges.hpp",
          "MergeCoincidentMeshEdges", "m_Mesh", Guard::HostOutlivesUs,
          "ported UE code (P12, GeometryCore), kept in UE's shape: the constructor takes the mesh it merges edges of (DynamicMesh3*, non-owning); built at the call site and applied in-place, never stored beyond that call" },
        { "Desert/Desert/Source/Engine/Geometry/UECore/DynamicMesh/Operations/MergeCoincidentMeshEdges.hpp",
          "MergeCoincidentMeshEdges", "m_EdgesToMerge", Guard::HostOutlivesUs,
          "ported UE code (P12, GeometryCore), kept in UE's shape: optional, caller-owned std::unordered_set<int32_t>* naming the edge subset to merge (null means the whole mesh); the caller's set outlives the merge call that reads it" },
        { "Desert/Desert/Source/Engine/Geometry/UECore/DynamicMesh/Operations/OffsetMeshRegion.hpp",
          "OffsetMeshRegion", "m_Mesh", Guard::HostOutlivesUs,
          "ported UE code (P12, GeometryCore), kept in UE's shape: an offset/extrude operation built at the call site over the caller's DynamicMesh3* and applied in-place; the operation object does not outlive that call" },
        ...
        { "Desert/Desert/Source/Engine/Geometry/UECore/DynamicMesh/Operations/GroupEdgeInserter.hpp",
          "EdgeLoopInsertionParams", "Mesh", Guard::CallScoped,
          "ported UE code (P12h, GeometryCore), kept in UE's shape: an GroupEdgeInserter argument pack built at the call site; the caller's DynamicMesh3* (non-owning) that InsertEdgeLoops edits in-place" },
        { "Desert/Desert/Source/Engine/Geometry/UECore/DynamicMesh/Operations/GroupEdgeInserter.hpp",
          "EdgeLoopInsertionParams", "Topology", Guard::CallScoped,
          "ported UE code (P12h, GeometryCore), kept in UE's shape: an GroupEdgeInserter argument pack built at the call site; the caller's GroupTopology* (non-owning) over that mesh, rebuilt in-place by InsertEdgeLoops" },
        { "Desert/Desert/Source/Engine/Geometry/UECore/DynamicMesh/Operations/GroupEdgeInserter.hpp",
          "EdgeLoopInsertionParams", "SortedInputLengths", Guard::CallScoped,
          "ported UE code (P12h, GeometryCore), kept in UE's shape: an GroupEdgeInserter argument pack built at the call site; the caller's sorted std::vector<double> of loop positions (non-owning), read during InsertEdgeLoops only" },
        { "Desert/Desert/Source/Engine/Geometry/UECore/DynamicMesh/Operations/GroupEdgeInserter.hpp",
          "GroupEdgeInserterOptionalOutputParams", "NewEidsOut", Guard::CallScoped,
          "ported UE code (P12h, GeometryCore), kept in UE's shape: an GroupEdgeInserter argument pack built at the call site; optional caller-owned std::unordered_set<int32_t>* out-parameter (null means not wanted), written during the insert call only" },
        { "Desert/Desert/Source/Engine/Geometry/UECore/DynamicMesh/Operations/GroupEdgeInserter.hpp",
          "GroupEdgeInserterOptionalOutputParams", "ChangedTidsOut", Guard::CallScoped,
          "ported UE code (P12h, GeometryCore), kept in UE's shape: an GroupEdgeInserter argument pack built at the call site; optional caller-owned std::unordered_set<int32_t>* out-parameter (null means not wanted), written during the insert call only" },
        { "Desert/Desert/Source/Engine/Geometry/UECore/DynamicMesh/Operations/GroupEdgeInserter.hpp",
          "GroupEdgeInserterOptionalOutputParams", "ProblemGroupEdgeIDsOut", Guard::CallScoped,
          "ported UE code (P12h, GeometryCore), kept in UE's shape: an GroupEdgeInserter argument pack built at the call site; optional caller-owned std::unordered_set<int32_t>* out-parameter (null means not wanted), written during InsertEdgeLoops only" },
        { "Desert/Desert/Source/Engine/Geometry/UECore/DynamicMesh/Operations/GroupEdgeInserter.hpp",
          "GroupEdgeInsertionParams", "Mesh", Guard::CallScoped,
          "ported UE code (P12h, GeometryCore), kept in UE's shape: an GroupEdgeInserter argument pack built at the call site; the caller's DynamicMesh3* (non-owning) that InsertGroupEdge edits in-place" },
        { "Desert/Desert/Source/Engine/Geometry/UECore/DynamicMesh/Operations/GroupEdgeInserter.hpp",
          "GroupEdgeInsertionParams", "Topology", Guard::CallScoped,
          "ported UE code (P12h, GeometryCore), kept in UE's shape: an GroupEdgeInserter argument pack built at the call site; the caller's GroupTopology* (non-owning) over that mesh, rebuilt in-place by InsertGroupEdge" },
        { "Desert/Desert/Source/Engine/Geometry/UECore/DynamicMeshEditor.hpp",
          "DynamicMeshEditor", "m_Mesh", Guard::HostOutlivesUs,
          "ported UE code (P12, GeometryCore), kept in UE's shape: the constructor takes the mesh it edits (DynamicMesh3*, non-owning); built at the call site for one edit and applied in-place, never stored beyond it" },
        { "Desert/Desert/Source/Engine/Geometry/UECore/MeshBoundaryLoops.hpp",
          "MeshBoundaryLoops", "m_Mesh", Guard::HostOutlivesUs,
        ...
        { "Desert/Desert/Source/Engine/Geometry/UECore/MeshRegionBoundaryLoops.hpp",
          "MeshRegionBoundaryLoops", "m_Mesh", Guard::HostOutlivesUs,
        ...
        { "Desert/Desert/Source/Engine/Geometry/UECore/Selections/MeshConnectedComponents.hpp",
          "MeshConnectedComponents", "m_Mesh", Guard::HostOutlivesUs,
        ...
        { "Editor/Source/Editor/Panels/TextureViewer/TextureViewerDocument.hpp",
          "TextureViewerDocument", "m_Assets", Guard::HostOutlivesUs,
          "EditorLayer owns the AssetManager and declares it BEFORE m_OpenDocuments, which holds every open document, so the viewer is destroyed first (members die in reverse declaration order); EditorLayerDeclaresItsHostsBeforeItsPanels asserts that order for documents rather than trusting it" },
        { "Editor/Source/Editor/Panels/SkyboxViewer/SkyboxViewerDocument.hpp",
          "SkyboxViewerDocument", "m_Assets", Guard::HostOutlivesUs,
          "EditorLayer owns the AssetManager and declares it BEFORE m_OpenDocuments, which holds every open document, so the viewer is destroyed first (members die in reverse declaration order); EditorLayerDeclaresItsHostsBeforeItsPanels asserts that order for documents rather than trusting it" },
        { "Editor/Source/Editor/Panels/StaticMeshViewer/StaticMeshViewerDocument.hpp",
          "StaticMeshViewerDocument", "m_Assets", Guard::HostOutlivesUs,
          "EditorLayer owns the AssetManager and declares it BEFORE m_OpenDocuments, which holds every open document, so the viewer is destroyed first (members die in reverse declaration order); EditorLayerDeclaresItsHostsBeforeItsPanels asserts that order for documents rather than trusting it" },
        ...
        { "Desert/Common/Source/Common/Json/Carry.cpp",
          "Frame", "Fresh", Guard::CallScoped,
          "an object inside the fresh document's yyjson tree, owned by that TextDocument (its shared parsed document), which MergeCarried's caller holds by reference for the whole call; the frame lives on MergeLevel's local stack" },
        { "Desert/Common/Source/Common/Json/Carry.cpp",
          "Frame", "Source", Guard::CallScoped,
          "an object inside the loaded document's yyjson tree, owned by that TextDocument, which MergeCarried's caller holds by reference for the whole call; the frame lives on MergeLevel's local stack" },
        { "Desert/Common/Source/Common/Json/Carry.cpp",
          "Frame", "Merged", Guard::CallScoped,
          "an object of the mutable merge document, freed only with that mutable document (MergeCarried's local MutDoc, which outlives the MergeLevel call); the frame lives on MergeLevel's local stack" },
        { "Desert/Desert/Source/Engine/Reflection/ReflectionSerializer.cpp",
          "Frame", "Type", Guard::CallScoped,
          "the TypeInfo of the struct this frame walks: owned by the process-wide ReflectionRegistry (or the caller's TypeInfo, which the call borrows); the frame lives on SerializeReflected/DeserializeReflected's local stack and dies before the call returns" },
        { "Desert/Desert/Source/Engine/Reflection/ReflectionSerializer.cpp",
          "Frame", "Base", Guard::CallScoped,
          "the first byte of the object (or nested struct inside it) this frame walks: the caller's object, borrowed for the one SerializeReflected/DeserializeReflected call; the frame lives on that call's local stack" },
        { "Desert/Desert/Source/Engine/Reflection/ReflectionSerializer.cpp",
          "Frame", "Field", Guard::CallScoped,
          "the parent's FieldInfo the finished struct is written under, an element of the parent TypeInfo's Fields (registry-owned, not mutated during a walk); the frame lives on SerializeReflected's local stack" },
