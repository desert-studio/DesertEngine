...
namespace Desert::Editor
{
    ...
    // FoldExternalLODMeshes) are left as they are.
    ...
    // simplifies the LOD chain. The output is the MeshBinary container (EncodeMeshBinary) - the DDC value.
    // Its header GUID is null: identity lives in the asset, and a derived entry shared by two assets with the
    // same source cannot name either of them.
    // Refused, naming the reason: a skinned asset (the skeleton's form is AF4f's decision), a triangle whose
    // material ID has no slot, and everything ToMeshAssetData refuses (colour layer, second UV layer).
