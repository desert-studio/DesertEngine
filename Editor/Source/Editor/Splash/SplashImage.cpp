    ...
    Common::ResultStr<SplashPixels> LoadSplashPixels( const std::filesystem::path& cookedTex )
    {
        ...
        if ( !std::filesystem::is_regular_file( cookedTex, ec ) )
            return Common::MakeFormattedError<SplashPixels>(
                 "'{}' is missing: it is a committed engine resource "
                 "(Editor/Resources/Splash), regenerated by `TextureCook Desert.deproj --emit "
                 "Resources/Splash/Splash.detex Resources/Splash/Splash.tex`",
                 cookedTex.string() );
